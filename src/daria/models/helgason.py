...
class HelgasonModel:
    '''
    Helgason et al. 2012 IGL model
    https://ui.adsabs.harvard.edu/#abs/2012ApJ...752..113H/abstract

    Note that in H12, luminosity function is in units of 
    [1/mag/(Mpc/h)^3] NOT [h^3/mag/Mpc^3], so all the LF results carry
    the h^3 units. The intensity calculation taking this into account.

    Code provided by Yun-Ting Cheng, as used in Cheng and Chang 2022, with
    slight modifications:
    https://iopscience.iop.org/article/10.3847/1538-4357/ac3aee
    '''
    def __init__(self):
        self.get_params()
        self.hmf = ct.init_hmf(Mmin=8,Mmax=15)
    ...
    def get_params(self):
        ...
        wleff_arr = [0.15,0.36,0.45,0.55,0.65,0.79,0.91,1.27,1.63,2.20,3.60,\
                     4.50]
        ...
        phi0_arr = [2.43,5.46,3.41,2.42,2.25,2.05,2.55,2.21,1.91,2.74,3.29,\
                    3.29]
    ...
    def LF_band(self, z, bandidx, M_arr = np.arange(-25,-12,0.01)):
        ...
        phi_arr = 0.4 * np.log(10) * phistr * \
            (10**(0.4 * (Mstr - M_arr)))**(alpha + 1) \
                * np.exp(-10**(0.4 * (Mstr - M_arr)))
        
        return M_arr, phi_arr
        
    def LF_wl(self, z, wl_rf, M_arr = np.arange(-25,0,0.01)):
        '''
        rest frame luminosity function [#/mag/Mpc^3] at (rest frame wl) wl_rf
        [um]
        ...
        if wl_rf < wl_eff_arr[0]:
            ...
            idx0, idx1 = np.where(wl_eff_arr>wl_rf)[0][0]-1, \
                np.where(wl_eff_arr>wl_rf)[0][0]
    ...
    def LF_wl_limits(self, z, wl_obs, m_min = -np.inf, m_max = np.inf, 
        m_th_ref=None, wl_obs_th_ref=1, ng_shot=None,
        M_arr = np.arange(-25,0,0.01)):
        '''
        return LF with mag limit constraints
        '''
        if z==0:
            return 0.
        if m_min == m_max:
        ...
        if ng_shot is not None:
            # integrate btwn m_min (unmasked gals) and m_max (gals in tracer)
        ...
        DM = 5 * np.log10((self.hmf.cosmo.luminosity_distance(z) / \
                           (10 * u.pc)).decompose()).value
        ...
        return M_arr, m_arr, Fnu_arr, phi_arr
    ...
    def get_mask_th(self, z, wl_obs, m_th_ref, wl_obs_th_ref):
        ...
        phi_arr_ref = self.LF_wl(z, wl_obs_th_ref/(1+z), \
                                 M_arr)[1] # [#/mag/(Mpc/h)^3]
    ...
    def dN_dsr_dz(self, z, wl_obs, m_min=0, m_max=15, 
        ...
        dchi_dz = (const.c / self.hmf.cosmo.H(z)).to(u.Mpc).value
        dMpc2_dsr = (self.hmf.cosmo.kpc_comoving_per_arcmin(z).\
                     to(u.Mpc/u.rad).value)**2
    ...
    def dnuInu_dz(self, z, wl_obs, m_min = -np.inf, m_max = np.inf, 
        ...
        Fnu_arr = 3631 * 10**(-m_arr / 2.5) # [Jy]
        phi_arr = self.LF_wl(z, wl_obs/(1+z), M_arr)[1] # [#/mag/Mpc^3]
        dInu_dz = np.sum(phi_arr * Fnu_arr) * dm * dchi_dz \
            * dMpc2_dsr # [Jy/sr]
        dnuInu_dz = (nu_obs * dInu_dz)*(u.Hz * u.Jy / u.sr).\
            to(u.nW/u.m**2/u.sr)
    ...
    def nuInu(self, wl_obs, zmin=0, zmax=6, **kwargs):
        '''
        Total EBL nuInu[nW/m2/sr] at obs wl wl_obs[um] 
        from redshift range (zmin, zmax)
        '''
        z_arr = np.linspace(zmin, zmax, 100)
        dz = z_arr[1] - z_arr[0]
        if zmin==0:
    ...
    def dClsh_dz(self, z, wl_obs, m_min = 13.5, 
        ...
        r'''
        integrate 2nd moment of Helgason LF to get 
        dCl_sh / dz [(nW/m2/sr)^2 sr].
        ...
        Fnu_arr = 3631 * 10**(-m_arr / 2.5) # [Jy]
        phi_arr = self.LF_wl(z, wl_obs/(1+z), M_arr)[1] # [#/mag/Mpc^3]
        dClsh_dz = np.sum(phi_arr * Fnu_arr**2 * nu_obs**2) * dm * \
            dchi_dz * dMpc2_dsr # [Jy/sr]
        dClsh_dz = (dClsh_dz)*((u.Hz * u.Jy)**2 / u.sr).\
            to((u.nW/u.m**2)**2/u.sr)

        return dClsh_dz
    
    def Clsh(self, wl_obs, zmin=0, zmax=6, **kwargs):
        '''
        Total Cl_shot [(nW/m2/sr)^2 sr] at obs wl wl_obs[um] 
        from redshift range (zmin, zmax)
        '''
    ...
    def Clsh_filter_weighted(self,filt_wls,filt_trans,verbose=False,**kwargs):
        ...
        for i,(wl, Rsq, dwl) in enumerate(zip(wls, Rsqs, dwls)):
            ...
            if verbose:
                print(
                    'Filter weighted Clsh %d/%d (%.3f um, Clsh=%.2e)'\
                        %(i,len(wls), wl, Clsh)
                    )
    ...
    def bI(self,z,HOD_M_min=1e9,HOD_M_sat=5e10,HOD_sigma_log_M=0.2,\
           HOD_alpha_s=1):
        '''
        Bias param from Helgason et al. 2012 model.

        In Helgason model, the bias is assumed to be only depend on redshift
        and not on the wavelength, halo mass, or other properties.
        '''
        self.hmf.update(z=z)
        Mh_arr = self.hmf.m # [Msun/h]
        h = self.hmf.cosmo.h
        dlnMh = np.log(Mh_arr[1]) - np.log(Mh_arr[0])
        Nc_arr = 0.5 * (1 + erf((np.log10(Mh_arr/h) \
                                 - np.log10(HOD_M_min))/HOD_sigma_log_M))
        Ns_arr = 0.5 * (1 + erf((np.log10(Mh_arr/h) \
                                 - np.log10(2*HOD_M_min))/HOD_sigma_log_M)) \
                                    * (Mh_arr/h/HOD_M_sat)**HOD_alpha_s
        Ngal_arr = Nc_arr + Ns_arr

        dndlnm_arr = self.hmf.dndlnm
        b_arr = ct.get_halo_bias(self.hmf,z)
